...
public class JsonYamlValidatorMojoIncludeExcludeTest extends AbstractMojoTestCase {
    ...
    public void testIncludeExcludePatterns() throws Exception {
        ...
        assertEquals("Mojo should find exactly " + EXPECTED_INCLUDED_FILES
                + " files after applying include/exclude patterns", EXPECTED_INCLUDED_FILES, fileCount);
        ...
        for (File file : files) {
            ...
            System.out
                    .println("  - " + relativePath
                            + (shouldBeExcluded ? " [SHOULD BE EXCLUDED: " + reason + "]" : " [OK]"));
        }
    ...
    public void testVerifyTestDirectoryStructure() throws Exception {
        ...
        try (Stream<Path> paths = Files.walk(testDir.toPath())) {
            List<Path> files = paths.filter(Files::isRegularFile).filter(p -> {
